...
> 名称全部采用**国服官方译名**（狂徒 / 幻影 / 冥驹 / 上古龙炎 / 掠影…），同时显示国际服英文名；价格使用**国服点券**（1 元 ≈ 10 点券），并在详情里附国际服 VP 对照。

## 功能

- 🔥 **当前礼包轮换**：每日更新脚本解析 valohub 商店快照，展示正在轮换的礼包、含皮肤明细、原价/折后价与折扣
- 🌙 **夜市活动**：基于官方历史公告锚点推算排期（进行中 / 下次开启倒计时），并内置「模拟我的夜市」——规则依据 [官方 wiki](https://wiki.playvalorant.com/en-us/Night.Market)：随机 6 款、折扣 10%~49%、品质仅**精选/豪华/卓越**（传奇与终极不进）、**近战可进但仅限 ≤3550 VP 的系列刀**、不含战队标配(VCT)/冠军赛/通行证/挂件皮肤、需上线满 2 个幕、至少 2 款为「近战或卓越」、同武器最多 2 款
- 🗂 **皮肤图鉴**：1,300+ 款皮肤，支持按武器类型 / 品质 / 系列 / 中英文名关键词筛选；点击皮肤卡片弹出详情，可切换等级查看**特效动画视频**（检视、枪口特效、终结特效）与全部配色
- 💎 **国服点券价格**：枪械档位为**国服商城实测**（精选 690 / 豪华 890 / 卓越 1290 / 传奇 1590 / 终极 1790）；近战按国服"同品质枪械价 ×2"（实测：卓越刀 2580、传奇刀 3180），高价刀（掠影/离子/鬼/魔幻朋克等系列，国际服 4350 VP 档）按 3180 点券计
- ⏰ **每日更新**：`scripts/update.mjs` 无依赖抓取全部数据（含国服简中语言包），支持 Windows 计划任务或 GitHub Actions 每日自动执行
...
| 数据 | 来源 | 说明 |
| --- | --- | --- |
| 皮肤 / 系列 / 武器 / 礼包名称 | [valorant-api.com](https://valorant-api.com) `?language=zh-CN` 语言包 | **国服官方译名**，同时保留英文名 |
| 品质 / 主题 / 特效视频 / 贴图 | 同上（官方客户端数据快照） | 视频为各等级检视、枪口特效、终结动画 |
| 当前商店轮换礼包 | [valohub.co/store](https://valohub.co/store)（公开页面解析） | 每日脚本尽力解析（带重试）；失败时自动回退「近期礼包精选」 |
| 夜市资格规则 | [官方 wiki · Night.Market](https://wiki.playvalorant.com/en-us/Night.Market) | 品质仅精选/豪华/卓越；近战限 ≤3550 VP 的系列刀；排除战队标配、冠军赛、通行证、挂件 |
| 夜市排期 | 官方公告历史锚点 + 间隔推算 | 未来场次为**推算值**，仅供参考 |
| 个人夜市内容 | —— | 公开接口无法获取（需玩家授权），看板内置**随机模拟器**代替 |
| 价格 | 国服商城实测档位 + 近战换算 | 枪械为实测（精选 690 / 豪华 890 / 卓越 1290 / 传奇 1590 / 终极 1790，塑水宗 1690、源能者 2190）；近战按同品质 ×2；礼包价为估算，实时礼包按快照折扣计算 |

**免责声明**：本站为社区向非官方工具，与 Riot Games 无关联；VALORANT 及相关素材版权归 Riot Games 所有。国服译名取自官方客户端简中语言包，若与游戏内显示不一致，以游戏内为准。

## 自定义

- `scripts/update.mjs` 中可编辑：
  - `FEATURED_ORDER` —— 「近期礼包精选」的排序（按**英文名**，新 → 旧），新礼包发布后按英文名插入最前即可
  - `NM_ANCHORS` —— 夜市真实日期锚点（每次官方公布后追加一条，推算会更准）
  - `GUN_PRICE_CN` / `MELEE_PRICE_CN` / `CN_THEME_OVERRIDE` —— 国服点券价格档位与特例（改这里即可修正价格）
- 皮肤卡片默认按品质排序，图鉴页支持中英文名搜索与筛选
